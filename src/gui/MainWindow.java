...
public class MainWindow {
    ...
    private void foodsPopulate(String name) {
        java.util.List<Food> food;

        food = foodsRead("fittracker_foods.json");


        java.util.List<Food> groupedFoods = food.stream().filter(e -> e.getName() == name).collect(Collectors.toList());

        JPanel listPanel = new JPanel();
        listPanel.setLayout(new BoxLayout(listPanel, BoxLayout.Y_AXIS));

        int totalHeight = 0;
        for (Food foods : groupedFoods) {
            JButton button = new JButton(foods.getName());
            button.setMaximumSize(new Dimension(Integer.MAX_VALUE, 100));
            button.setHorizontalAlignment(SwingConstants.CENTER);
            button.setForeground(Color.white);
            button.setFont(button.getFont().deriveFont(button.getFont().getStyle() | Font.BOLD, button.getFont().getSize() + 10f));
            button.addActionListener(e -> foodSelection(foods));
            listPanel.add(button);
            totalHeight += 100;
        }

        listPanel.setPreferredSize(new Dimension(foodFoodsScrollPanel.getViewport().getWidth(),totalHeight));

        foodFoodsScrollPanel.setViewportView(listPanel);
        foodFoodsScrollPanel.revalidate();
        foodFoodsScrollPanel.repaint();
    ...
    private void listofFoods(ActionEvent e) {
        ...
        foodsPopulate("");
        foodFoodsScrollPanel.setBounds(new Rectangle(new Point(6, 106), foodFoodsScrollPanel.getPreferredSize()));
        foodFoodsScrollPanel.setVisible(true);
        mainMenuPanel.add(foodFoodsScrollPanel, BorderLayout.CENTER);
        mainMenuPanel.revalidate();
        mainMenuPanel.repaint();
    }
    ...
    private void initComponents() {
        ...
        //======== mainMenuFoodsPanel ========
        {
            ...
            foodSelectButton.addActionListener(e -> listofFoods(e));
    ...
    private JScrollPane foodFoodsScrollPanel;
